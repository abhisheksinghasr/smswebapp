pipeline {
    ...
    environment {
        ...
        // List of servers and their respective instance IDs
        SERVERS = [
            ...
            steps {
                script {
                    for (server in SERVERS) {
                        ...
                        stage("Deregister ${remoteServer} from Target Group") {
                            echo "🚫 Deregistering instance ${instanceId} from Target Group..."
                            def deregisterCmd = """
                                aws elbv2 deregister-targets --target-group-arn ${TARGET_GROUP_ARN} --targets Id=${instanceId} --region ${AWS_REGION}
                            """
                            bat deregisterCmd
                        ...
                        stage("Register ${remoteServer} Back to Target Group") {
                            echo "🔄 Registering instance ${instanceId} back to Target Group..."
                            def registerCmd = """
                                aws elbv2 register-targets --target-group-arn ${TARGET_GROUP_ARN} --targets Id=${instanceId} --region ${AWS_REGION}
                            """
                            bat registerCmd

                            echo "⏳ Waiting for instance ${instanceId} to become healthy..."
                            def waitForHealthyCmd = """
                                aws elbv2 wait target-in-service --target-group-arn ${TARGET_GROUP_ARN} --targets Id=${instanceId} --region ${AWS_REGION}
                            """
